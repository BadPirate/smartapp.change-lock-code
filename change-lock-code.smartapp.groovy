...
preferences {
    ...
    page(name: "page4", title: "Set user preferences", nextPage: "page4", install: true, uninstall: true) {
        ...
        section("Remote User 7") {
            input "username7", "text", title: "Name for User", required:false
            paragraph "Code can be set remotely using Web Services"
            input "sendCode7", "enum", title: "Send notification when users code is used", metadata:[values:["Yes","No"]], required:false
        }
    }
}
...
void setCode7() {
    def msg = "Remote Slot 7 remote set to code $params.code"
    log.info msg
    state.code7 = params.code
    updateCode(7)
}
